...
function checkYear(str) {
  return (/^(1900|19\d\d|20\d\d|2100)$/).test(str);
}
...
function checkDomainUrl(str) {
  return (/^(http:|https:)\/\/([a-z0-9-_]+\.)*[a-zA-Z0-9-_]+\.[a-z]{2,}$/).test(str);
}
...
function createLinksFromDomains(str) {
  ...
  str.match(regexp).forEach((item) => str = str.replace(item, `<a href="${item}" target="_blank">${item.replace(/^(http:|https:)\/\//gi, '')}</a>`));
  return str;
}
